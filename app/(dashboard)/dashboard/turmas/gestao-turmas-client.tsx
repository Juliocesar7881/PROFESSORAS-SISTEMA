...
export function GestaoTurmasClient() {
  ...
  return <div className="mx-auto max-w-[1300px] space-y-4">
    <div className="flex flex-col gap-3 rounded-lg border border-[#e8e3f0] bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
    ...
    {loading ? <div className="grid min-h-48 place-items-center rounded-lg border border-[#e8e3f0] bg-white"><Loader2 className="size-6 animate-spin text-[#6757c8]" /></div> : showTrash ? <div className="grid gap-4 lg:grid-cols-2">
      ...
      {turmas.map((turma) => <section key={turma.id} className="rounded-lg border border-[#e8e3f0] bg-white">
        ...
        <div className="grid gap-2 p-4 sm:grid-cols-2 lg:grid-cols-3">{(childrenByClass.get(turma.id) ?? []).map((child) => <article key={child.id} className="flex items-center justify-between gap-2 rounded-md border border-[#e8e3f0] bg-[#ffffff] p-3"><div className="min-w-0"><p className="truncate font-bold text-[#17213f]">{child.nome}</p><p className="truncate text-xs text-[#6d6c82]">{child.contexto || (child.dataNasc ? `Nascimento: ${new Date(child.dataNasc).toLocaleDateString("pt-BR")}` : "Sem contexto adicional")}</p></div><div className="flex shrink-0"><button type="button" onClick={() => openCriancaEdit(child)} className="inline-flex size-8 items-center justify-center rounded-md text-[#6757c8] hover:bg-[#f3f0ff]" aria-label="Editar crianca"><Pencil className="size-4" /></button><button type="button" onClick={() => remove("criancas", child.id)} className="inline-flex size-8 items-center justify-center rounded-md text-red-600 hover:bg-red-50" aria-label="Mover crianca para lixeira"><Trash2 className="size-4" /></button></div></article>)}<button type="button" onClick={() => { setCriancaForm({ nome: "", turmaId: turma.id, dataNasc: "", contexto: "" }); setEditingCrianca(null); setShowCriancaForm(true); }} className="flex min-h-16 items-center justify-center gap-2 rounded-md border border-dashed border-[#dcd3f7] text-sm font-bold text-[#6757c8] hover:bg-[#f8f6ff]"><Plus className="size-4" /> Adicionar crianca</button></div>
      </section>)}
    </div>}

    {showTurmaForm ? <div className="fixed inset-0 z-50 grid place-items-end bg-[#17213f]/45 p-0 sm:place-items-center sm:p-4"><div className="w-full max-w-xl rounded-t-lg bg-white p-5 sm:rounded-lg"><div className="flex items-center justify-between"><h2 className="font-heading text-xl text-[#17213f]">{editingTurma ? "Editar turma" : "Nova turma"}</h2><button onClick={resetTurma} className="inline-flex size-9 items-center justify-center rounded-md border border-[#e8e3f0]"><X className="size-4" /></button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label className="sm:col-span-2"><span className="pf-label">Nome da turma *</span><input className="pf-input h-11" value={turmaForm.nome} onChange={(event) => setTurmaForm({ ...turmaForm, nome: event.target.value })} autoFocus /></label><label><span className="pf-label">Etapa / faixa etaria</span><input className="pf-input h-11" value={turmaForm.faixaEtaria} onChange={(event) => setTurmaForm({ ...turmaForm, faixaEtaria: event.target.value })} /></label><label><span className="pf-label">Turno</span><input className="pf-input h-11" value={turmaForm.turno} onChange={(event) => setTurmaForm({ ...turmaForm, turno: event.target.value })} /></label><label><span className="pf-label">Instituicao</span><input className="pf-input h-11" value={turmaForm.instituicao} onChange={(event) => setTurmaForm({ ...turmaForm, instituicao: event.target.value })} /></label><label><span className="pf-label">Ano</span><input type="number" className="pf-input h-11" value={turmaForm.ano} onChange={(event) => setTurmaForm({ ...turmaForm, ano: event.target.value })} /></label></div><div className="mt-5 flex justify-end gap-2"><Button variant="outline" onClick={resetTurma}>Cancelar</Button><Button onClick={saveTurma} disabled={saving}>{saving ? <Loader2 className="size-4 animate-spin" /> : null} Salvar turma</Button></div></div></div> : null}
    {showCriancaForm ? <div className="fixed inset-0 z-50 grid place-items-end bg-[#17213f]/45 p-0 sm:place-items-center sm:p-4"><div className="w-full max-w-xl rounded-t-lg bg-white p-5 sm:rounded-lg"><div className="flex items-center justify-between"><h2 className="font-heading text-xl text-[#17213f]">{editingCrianca ? "Editar crianca" : "Nova crianca"}</h2><button onClick={resetCrianca} className="inline-flex size-9 items-center justify-center rounded-md border border-[#e8e3f0]"><X className="size-4" /></button></div><div className="mt-4 grid gap-3 sm:grid-cols-2"><label><span className="pf-label">Nome *</span><input className="pf-input h-11" value={criancaForm.nome} onChange={(event) => setCriancaForm({ ...criancaForm, nome: event.target.value })} autoFocus /></label><label><span className="pf-label">Turma *</span><select className="pf-select h-11" value={criancaForm.turmaId} onChange={(event) => setCriancaForm({ ...criancaForm, turmaId: event.target.value })}><option value="">Selecione</option>{turmas.map((item) => <option key={item.id} value={item.id}>{item.nome}</option>)}</select></label><label><span className="pf-label">Nascimento</span><input type="date" className="pf-input h-11" value={criancaForm.dataNasc} onChange={(event) => setCriancaForm({ ...criancaForm, dataNasc: event.target.value })} /></label><label className="sm:col-span-2"><span className="pf-label">Contexto</span><Textarea rows={4} value={criancaForm.contexto} onChange={(event) => setCriancaForm({ ...criancaForm, contexto: event.target.value })} /></label></div><div className="mt-5 flex justify-end gap-2"><Button variant="outline" onClick={resetCrianca}>Cancelar</Button><Button onClick={saveCrianca} disabled={saving}>{saving ? <Loader2 className="size-4 animate-spin" /> : null} Salvar crianca</Button></div></div></div> : null}
  </div>;
}
